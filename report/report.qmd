...
Il modello cinematico del veicolo di tipo uniciclo è fondamentale per lo sviluppo dell'algoritmo. Di seguito vengono definiti lo stato, i vincoli e le equazioni di controllo che descrivono il movimento del robot.

## Stato del robot
Lo stato del veicolo sul piano bidimensionale è descritto dal vettore delle coordinate generalizzate $q$:
$$
q = \begin{bmatrix} x \\ y \\ \theta \end{bmatrix}
$$
dove le coordinate $(x, y)$ rappresentano la posizione del punto medio dell'asse delle ruote di trazione, e l'angolo $\theta$ rappresenta l'orientamento (heading) del veicolo rispetto all'asse X orizzontale del sistema di riferimento globale.
...
Esprimendo il vincolo nella forma pfaffiana e calcolandone lo spazio nullo per trovare le direzioni di movimento consentite, il modello cinematico vincolato assume la forma $\dot{q} = G(q)u$. 
