...
L'IMU è un sensore propriocettivo che misura accellerazioni lungo gli assi cardinali, nel caso della versioni planare le misurazioni ottenute sono:
...
- **Bias** un errore sistematico a lenta variazione temporale. Nel codice, il bias è modellato come un *Random Walk*, ovvero il bias attuale è uguale al bias precedente piu un piccolo rumore gaussiano. 
...
## Camera
La *Camera* all'interno della simulazione viene semplificata usando il modello *pin-hole*; questa riconosce punti di riferimento posizionati all'interno dell'ambiente, chiamati *landmark*, la cui posizione nel frame world $<w>$ e' conosciuta a priori. 
...
La classe esegue un'operazione di "anticipazione" delle formule dell'EKF:

1.*Predizione dello Stato e della Covarianza*, calcola in anticipo lo stato predetto (pred_state) e la propagazione della covarianza dell'errore (pred_P).  
2. *Calcolo della Covarianza dell'Innovazione* ($S$), utilizzando il modello di misura non lineare $h(x)$ e la sua Matrice Jacobiana $H$ calcolata nel punto di predizione, il filtro proietta l'incertezza dello stato nello spazio dei pixel e le somma il rumore intrinseco del sensore $R$. La formula applicata è esattamente: $S = H P^- H^T + R$.  
3. *Rejection*, poiché stiamo valutando l'errore in pixel su una singola dimensione ottica. La distanza di Mahalanobis diventa un semplice rapporto scalare: $D^2 = \frac{(z - \hat{z})^2}{S}$. Se questo scostamento è ritenuto statisticamente improbabile (supera la soglia $\chi^2$), la misurazione del landmark viene scartata prima che possa corrompere la stima dello stato dell'EKF.
...
for lm, z_true in zip(camera_measurements['landmarks'], camera_measurements['pixels']):
    ...
    # Calcolo della Distanza di Mahalanobis scalare 
    diff = float(z_true - z_pred)
    mahalanobis_sq = (diff ** 2) / S_i_scalar
    
    # Decisione basata sul Chi-Quadro
    if mahalanobis_sq <= self.chi2_threshold:
