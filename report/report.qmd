...
Questi due problemi sono, essenzialmente, l'uno l'opposto dell'altro.
...
Il modello dell'uniciclo è ampiamente utilizzato nel campo della robotica mobile, sia nell'automazione industriale che nella ricerca accademica, grazie al suo perfetto bilanciamento tra realismo fisico e complessità. 
Un vantaggio fondamentale di questo modello è la sua grande utilità pratica, le equazioni cinematiche che descrivono il movimento dell'uniciclo sono le stesse che governano la maggior parte dei veicoli cingolati e dei robot a guida differenziale. 
Questo significa che, in fase di implementazione pratica, gli ingressi di controllo teorici definiti per l'uniciclo possono essere facilmente tradotti nelle velocità di rotazione effettive della ruota destra e sinistra di un robot reale. Di conseguenza, l'uniciclo offre una base ideale che coniuga la semplicità teorica con una immediata utilità e applicabilità sperimentale.

## Stato del robot
Lo stato del veicolo sul piano bidimensionale è descritto dal vettore delle coordinate generalizzate $S$:
$$
S = \begin{bmatrix} x \\ y \\ \theta \end{bmatrix}
$$
dove le coordinate $(x, y)$ rappresentano la posizione del punto medio dell'asse delle ruote di trazione, e l'angolo $\theta$ rappresenta l'orientamento del veicolo rispetto all'asse X orizzontale del sistema di riferimento globale.

## Vincolo Cinematico
Il veicolo è soggetto a un vincolo cinematico non olonomo, il quale impone che l'uniciclo non possa traslare lateralmente lungo la direzione dell'asse delle ruote, quindi la velocità perpendicolare all'orientamento del veicolo deve essere nulla:
$$
\dot{x} \sin(\theta) - \dot{y} \cos(\theta) = 0
$$

## Equazioni di Stato e di Controllo
Il vettore degli ingressi di controllo è definito come $u = [u_1, u_2]^T$, dove $u_1$ rappresenta la velocità di avanzamento lineare e $u_2$ rappresenta la velocità angolare di sterzata. Le equazioni di stato che descrivono l'evoluzione delle coordinate nel tempo diventano quindi:
$$
\begin{bmatrix} \dot{x} \\ \dot{y} \\ \dot{\theta} \end{bmatrix} = \begin{bmatrix} \cos(\theta) & 0 \\ \sin(\theta) & 0 \\ 0 & 1 \end{bmatrix} 
\begin{bmatrix} u_1 \\ u_2 \end{bmatrix}
$$

Il modello esplicito risulta essere:

- $\dot{x} = u_1 \cos(\theta)$
- $\dot{y} = u_1 \sin(\theta)$
- $\dot{\theta} = u_2$

In fase di implementazione pratica, come nel caso di un robot a guida differenziale, questi ingressi di controllo sono legati alle velocità di rotazione della ruota destra ($\omega_r$) e sinistra ($\omega_l$) attraverso le relazioni $u_1 = \frac{r}{2}(\omega_r + \omega_l)$ e $u_2 = \frac{r}{L}(\omega_r - \omega_l)$, con $r$ raggio e $L$ la lunghezza di inteasse delle ruote.




## Modelli dei Sensori a bordo
...
questa coppia rappresenta una configurazione molto comune poiche' solitamente si tende ad accoppiare sensori propriocettivi con sensori esterocettivi per ragioni che verranno trattate in seguito.
...
$$
\begin{align*}
E(x(t)) &= E(\bar{x}(t) + \frac{1}{2} \epsilon t^2) \\
        &= E(\bar{x}(t)) + E(\frac{1}{2} t^2 E(\epsilon)) \\
        &= \bar{x}(t) + \frac{1}{2} t^2 E(\epsilon)
\end{align*}
$$
$$
\begin{align*}
E((x(t) - \bar{x}(t))^2) &= E((\frac{1}{2} \epsilon t^2)^2) \\
                        &= \frac{1}{4} t^4 E(\epsilon^2) \\
...
#### Proiezione e Utilizzo nell'EKF
...
Successivamente, per simulare la formazione dell'immagine sul sensore ottico, le coordinate vengono normalizzate rispetto alla profondità $x$ e moltiplicate per i parametri intrinseci della telecamera, ottenendo una coordinata in pixel $u$ affetta da rumore gaussiano.

Nel file `kalman_filter.py`, la fase di *Aggiornamento* utilizza queste letture in pixel per correggere lo stato. Il metodo `measurement_model()` replica esattamente il calcolo della proiezione ottica per calcolare la misura attesa $\hat{z}$. Nello stesso momento, la funzione `measurement_jacobian()` calcola la matrice $H$.
...
- *Aggiornamento*, il filtro confronta il dato reale con quello ottenuto dalla previsione. Attraverso un coefficiente chiamato *Guadagno di Kalman* il filtro stabilisce quanta fiducia dare alla predizione teorica e quanta alla misurazione reale, combinandole per ottenere la stima finale.
...
Per superare questa limitazione bisogna quindi utilizzare una variante del filtro chiamata *Filtro di Kalman Esteso (EKF)*, che permette di gestire sistemi non lineari linearizzando le equazioni attorno alla stima corrente dello stato.

## Filtro di Kalman esteso

Nella robotica i modelli cinematici, come quello dell'uniciclo, sono governati da leggi fortemente non lineari. Per questo è stata sviluppata una variante del filtro chiamata *EKF* (Extended Kalman Filter), che permette di propagare le covarianze e calcolare il guadagno utilizzando modelli linearizzati.
...
Per risolvere il problema sopra citato, il filtro degli outlier si basa sulla *Distanza di Mahalanobis*.
...
Sotto l'assunzione che il rumore del sistema sia bianco, la variabile statistica $D^2$ segue una distribuzione del *Chi-Quadro* ($\chi^2$) con un numero di gradi di libertà pari alla dimensionalità della misurazione.
...
- Se $D^2 \leq \text{soglia}$, il dato appartiene statisticamente alla distribuzione attesa. Viene accettato.
- Se $D^2 > \text{soglia}$, il dato è statisticamente troppo improbabile. Viene classificato come outlier e rigettato.
...
Il concetto chiave in questa fase è l'*Innovazione* (o residuo), definita come la differenza tra la misurazione reale $z$ ottenuta dal sensore e la misurazione attesa $\hat{z}$. Quest'ultima viene calcolata proiettando lo stato predetto del robot nel dominio ottico del sensore tramite il modello di misura $h(x)$.

Per implementare questo controllo, la classe esegue un'operazione di "anticipazione" dell'EKF prima di procedere al vero aggiornamento dello stato, che cosiste nella seguenti operazioni:
...
Per dimostrare l'effettiva robustezza dell'algoritmo sviluppato, la simulazione è stata testata iniettando deliberatamente degli errori artificiali all'interno delle misurazioni dei sensori. I risultati seguenti mostrano il comportamento del sistema di rilevamento e la resilienza del Filtro di Kalman Esteso.

## Efficacia del filtro per la IMU
...
## Efficacia del filtro per la Telecamera
